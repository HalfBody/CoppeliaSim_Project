...
class Fuzzy():
    ...
    def get_lidar_data(self, Lidar_Data, mode):
        if mode == True: print('запуск лидара в управлении логикой')
        lidar_data = Lidar_Data
        
        if mode == True: print('lidar_data')
        
        r_temp, rf_temp, f_temp, lf_temp, l_temp = [], [], [], [], []

        one_of_five = math.floor(len(lidar_data)/5)
        lid_len = len(lidar_data)

        for i in range(len(lidar_data)):

            
            # разделение лидара на пять секторов по точкам
            if i >= 0 and i < one_of_five:
                if 0.0 < lidar_data[i] <= 1.5:
                    r_temp.append(lidar_data[i])
                else:
                    r_temp.append(1.5)
            elif i >= one_of_five and i < 2*one_of_five:
                if 0.0 < lidar_data[i] <= 1.5:
                    rf_temp.append(lidar_data[i])
                else:
                    rf_temp.append(1.5)
            elif i >= 2*one_of_five and i < lid_len - 2*one_of_five:
                if 0.0 < lidar_data[i] <= 1.5:
                    f_temp.append(lidar_data[i])
                else:
                    f_temp.append(1.5)
            elif i >= lid_len - 2*one_of_five and i < lid_len - one_of_five:
                if 0.0 < lidar_data[i] <= 1.5:
                    lf_temp.append(lidar_data[i])
                else:
                    lf_temp.append(1.5)
            elif i >= lid_len - one_of_five and i < lid_len:
                if 0.0 < lidar_data[i] <= 1.5:
        ...
        rf_data = min(rf_temp)
        r_data = min(r_temp)
        if mode == True: print('Минимальные значения детектирования для секторов лидара: ', [l_data, lf_data, f_data, rf_data, r_data])
        return [l_data, lf_data, f_data, rf_data, r_data]
        
        
    def fuz_log(self, Lidar_Data, target_angle, sim, mode):
        lidar_data = self.get_lidar_data(Lidar_Data, mode)
        #lidar_data = Lidar_Data
